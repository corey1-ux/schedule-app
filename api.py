...
def get_blocks():
    ...
    return jsonify([dict(b) for b in rows])
...
def get_block_events(block_id):
    ...
    grouped = {}
    for row in rows:
        grouped.setdefault((row["date"], row["skill_name"]), []).append(row["staff_name"])

    return jsonify([{
        "title": f"{sn}: {', '.join(names)}",
        "start": ds, "end": ds,
        "skill": sn, "staff": names, "source": "requests",
    } for (ds, sn), names in grouped.items()])
...
def _pay_periods(block_start_str, block_end_str):
    ...
    days_back    = (block_start.weekday() + 1) % 7
...
def validate_fte(block_id):
    ...
    for s in staff_rows:
        ...
        for i, (p_start, p_end) in enumerate(periods):
            ...
            if total > target:
                warnings.append({"type": "over",  "staff": s["name"],
                                  "message": f"{s['name']} is over FTE in {label} ({total}/{target})"})
            elif total < target:
                warnings.append({"type": "under", "staff": s["name"],
...
def run_optimize(block_id):
    try:
        ...
        return jsonify({"error": "OR-Tools not installed"}), 500
...
def get_optimized(block_id):
    ...
    return jsonify({"result": result, "unmet": unmet, "optimized_at": row["optimized_at"]})
...
def publish_block(block_id):
    with get_db() as conn:
        ...
        if is_republish:
            old = json.loads(last_pub["snapshot_json"])
            def to_tuples(snap):
                return {(d, sk, n) for d, skills in snap.items()
                        for sk, names in skills.items() for n in names}
